...
overlay_stuff_packet(struct outgoing_packet *packet, overlay_txqueue *queue, time_ms_t now, strbuf debug){
  ...
  while(frame){
    ...
    if (frame->packet_version<1 || frame->resend<=0 || packet->seq==-1)
      will_retransmit=0;
    //__android_log_print(ANDROID_LOG_DEBUG, "Leaf0805", "Leaf0805%s",packet->buffer->bytes);
...
overlay_fill_send_packet(struct outgoing_packet *packet, time_ms_t now, strbuf debug) {
  ...
  if(packet->buffer){
    ...
    if (times >= 9){
      ...
      dr = recv_remote_request();
    }
    /*__android_log_print(ANDROID_LOG_DEBUG, "aqua0711", "times : %d; role: %d", times, (dr==-1)?dr:dr+1);*/

    //dr = recv_remote_request();
    if(dr == GD){
      //__android_log_print(ANDROID_LOG_DEBUG, "aqua0711", "Overlay_queue, GD");
    }else if (dr == ND){
      //__android_log_print(ANDROID_LOG_DEBUG, "aqua0711", "Overlay_queue, ND");
    }else if (dr == FD) {
      //__android_log_print(ANDROID_LOG_DEBUG, "aqua0711", "Overlay_queue, FD");
      if (!GD_IP)
        //__android_log_print(ANDROID_LOG_DEBUG, "aqua0711", "GD_IP is null");
      if (!GD_Port)
        //__android_log_print(ANDROID_LOG_DEBUG, "aqua0711", "GD_Port is null");
      if (GD_IP && GD_Port) {
        //__android_log_print(ANDROID_LOG_DEBUG, "aqua0711", "before change: %s:%s", GD_IP, GD_Port);
        if (strcmp(GD_IP, "No") != 0 && strcmp(GD_Port, "GD") != 0) {
          //__android_log_print(ANDROID_LOG_DEBUG, "aqua0711", "change IP and Port");

          packet->destination->address.inet.sin_addr.s_addr = inet_addr(GD_IP);
          packet->destination->address.inet.sin_port = htons(atoi(GD_Port));
        }
      }
    }

    //__android_log_print(ANDROID_LOG_DEBUG, "aqua0711", "overlay_queue, overlay_fill_send_packet:from %s to %s by %s",alloca_socket_address(&packet->destination->interface->address),alloca_socket_address(&packet->destination->address), packet->destination->interface->name);
...
int recv_remote_request(void) {
  int localsocket, len;
  struct sockaddr_un remote;
  if ((localsocket = socket(AF_UNIX, SOCK_STREAM, 0)) == -1) {
    return -1;
  }
  char *name = "wifi.ap.info";
  remote.sun_path[0] = '\0';
  ...
  char msg[256];
  if (recv(localsocket, msg, sizeof(msg), 0) == -1) {
    return -1;
  }
  char *role = strtok(msg, "#");
  close(localsocket);
  if(role) {
    /*__android_log_print(ANDROID_LOG_DEBUG, "aqua0711",
                        "Overlay_queue, recv_remote_request, ROLE: %s",
                        role);*/

    if(strcmp(role,"1") == 0){
            //__android_log_print(ANDROID_LOG_DEBUG, "aqua0711", "Overlay_queue, 1");
        return GD;
    }
    if(strcmp(role,"2") == 0){
            //__android_log_print(ANDROID_LOG_DEBUG, "aqua0711", "Overlay_queue, 2");
        return ND;
    }
    if (strcmp(role, "3") == 0) {

      strcpy(GD_IP,strtok(NULL, " "));
      //__android_log_print(ANDROID_LOG_DEBUG, "aqua0711","Overlay_queue, recv_remote_request, GD IP: %s", GD_IP);
      strcpy(GD_Port,strtok(NULL, ";"));
      //__android_log_print(ANDROID_LOG_DEBUG, "aqua0711","Overlay_queue, recv_remote_request, GD Port: %s", GD_Port);
      if (!GD_IP) {
        if (strcmp(GD_IP, "No") == 0) {
          //__android_log_print(ANDROID_LOG_DEBUG, "aqua0711", "Internal, GD IP: %s", GD_Port);
        }
      }
      if (!GD_Port) {
        if (strcmp(GD_Port, "GD") == 0) {
          //__android_log_print(ANDROID_LOG_DEBUG, "aqua0711", "Internal, GD Port: %s", GD_Port);
        }
      }
      return FD;
     }
