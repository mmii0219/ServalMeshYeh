...
int parseEnvelopeHeader(struct decode_context *context, struct overlay_interface *interface, 
  ...
  if (context->sender){
	...
	/*__android_log_print(ANDROID_LOG_DEBUG, "Leaf0506", "overlay_packetformats, parseEnvelopeHeader:SID:%s",alloca_tohex_sid_t(context->sender->sid));*/
		    ...
		    struct socket_address *recvaddr)
{
  ...
  */
  ...
  /*__android_log_print(ANDROID_LOG_DEBUG, "Leaf0506", "overlay_packetformats, packetOkOverlay:from %s",alloca_socket_address(recvaddr));*/
