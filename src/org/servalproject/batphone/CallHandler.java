...
public class CallHandler {
	...
	public static CallHandler dial(UnsecuredCall ui, Peer peer) throws IOException {
		...
		call.ui = ui;
		call.dial();
        Log.d(TAG, "Leaf_createcall() begin");
		return call;
	}
    public void dial() {
        ...
        initiated = true;
        monitor.sendMessageAndLog("call ",remotePeer.sid.toHex(), " ",main.getDid(), " ", did);
        Log.v(TAG, "test");
    }
	private static synchronized CallHandler createCall(Peer peer)
        ...
        Log.d(TAG, "Leaf_createcall() done");
	...
	private void startAudio() {
		try {
            Log.d(TAG, "Leaf_startAudio() begin");
			...
			audioRunning = true;
            Log.d(TAG, "Leaf_startAudio() done");
		} catch (Exception e) {
	...
	public boolean getSpeakerState(){
		if(am!=null){
			Log.d("aquariuskon1117", String.valueOf(am.isSpeakerphoneOn()));
